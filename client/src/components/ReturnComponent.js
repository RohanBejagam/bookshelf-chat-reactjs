...
function RenderIssue ({issue,i,returnBook}) {
    ...
    dates.push(deadline);
    const returnDate=issue.returned?new Date(Date.parse((issue.updatedAt))):(new Date(Math.max.apply(null,dates)));
...
render(){
    ...
    if (this.props.issues.isLoading) {
        ...
        <tbody>
            ...
            <br/>
            {/* <h6> Total Fine Collected as on today:  Rs. {totalFine} </h6> */}
            <br/>
            </div>
            </div>
            </div>
